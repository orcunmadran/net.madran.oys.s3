...
if( $_POST['Submit2'] )
{
	...
	if( $errMsg == '' )
	{
		...
		foreach($fld['err'] as $k => $v)
		{
			...
			$f = $stmt->process(utf8_encode($v['name']), utf8_encode($v['name']), $k);
			$value = $v['value'];
			if ( $value == $subst )
				$value = "";
			...
			$f = $stmt->process(utf8_encode($value), $disabled, $k, $_SESSION['session_inst']);
		}
	}
	unlink(APPDATA_DIR.'badwords'.'_'.$_SESSION['session_inst'].'.php');//delete file
}
