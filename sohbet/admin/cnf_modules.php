...
foreach($values as $k => $v)
{
	foreach($v as $key => $val)
	{
		...
		$return[$key]['name'] = performModuleName($all_modules[$key][0]);
		if(strpos(strtolower($all_modules[$key][0]), 'video') !== false || strpos(strtolower($all_modules[$key][0]), 'whiteboard') !== false )
		{
