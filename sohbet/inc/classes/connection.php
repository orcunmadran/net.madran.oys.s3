...
//		function Connection($id = null, $args = array()) {
		...
		function getAvailableRoom($roomId)
		{
			...
			$stmt = new Statement('SELECT id, ispermanent FROM '.$GLOBALS['fc_config']['db']['pref'].'rooms where instance_id = ?',51);
		...
		function process($req) {
			...
			if($req['c'] == 'lin')
			{
				...
				$this->doLogin($req['lg'], $req['ps'], $req['l'], $req['tz'], $req['r'], $req['bot_ip'], $req['session_inst']);// changed on 090706 for chat instances
		...
		function doLogin($login, $password, $lang, $tzoffset, $roomid = null, $bot_id = null, $session_inst = 1) {
