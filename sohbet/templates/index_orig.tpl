...
function popupLogin() {
	...
	var url = 'flashchat.php?username=' + username + '&password=' + password + '&lang=' + lang+'&session_inst='+document.login.session_inst.value;
	var options = 'width=' + width + ',height=' + height + ',top=' + y + ',left=' + x + ',resizable';
  ...
  <form action="flashchat.php" method="post" name="login">
    ...
    <tr>
      <td colspan="2" align="right"><table width="100%" border="0" cellspacing="0" cellpadding="2">
		  ...
		  <tr>
		  	<td><div align="right">Chat:</div></td>
			<td>
				<select name="session_chat" onchange='javascript:changeConf("{$str_chat}","{$str_inst}",session_chat,session_inst);'>      
					{foreach from=$data.chats key=key item=val}
		  ...
		  <tr>
            <td><div align="right">Instance:</div></td>
            <td>
